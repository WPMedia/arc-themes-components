...
export const SamplePagerButton = ({ index, onClick }) => {
	return (
		<Button type="button" className="custom-pager" onClick={onClick}>
			<span aria-hidden="true">&bull;</span>
			<span className="visually-hidden">Go to slide {index}</span>
		</Button>
	);
};
...
forward or backward. The container can take in markup of children div containers.
...
const Feature = () => (
	<Carousel>
		<Image src="my-image.jpg" alt="Image alt" />
		<Image src="my-image.jpg" alt="Image alt" />
		<Image src="my-image.jpg" alt="Image alt" />
		<Image src="my-image.jpg" alt="Image alt" />
	</Carousel>
...
```jsx
const SampleCard = ({ src }) => <Image src={src} />;
```

The examples used below will use the above `SampleCard` component for brevity.
...
Here is an example of a custom pager component. It is used in the `Custom Paging` story:
	...
	<Story name="Custom Paging">
		<Carousel
			sliderProps={{
				...
				slidesToScroll: 1,
				initialSlide: 0,
				customPaging: (i) => <SamplePagerButton index={i} />,
			}}
		>
			<SampleCard src="/camera.jpg" />
			<SampleCard src="/canyon.jpg" />
