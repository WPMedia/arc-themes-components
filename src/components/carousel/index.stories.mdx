...
Autoplay functionality that rotates the images within the carousel is also available. The autoplay feature is [accessible](https://www.w3.org/TR/wai-aria-practices-1.2/#carousel) for users who have opted into [`prefer-reduced-motion`](https://developer.mozilla.org/en-US/docs/Web/CSS/@media/prefers-reduced-motion) by not displaying the option, trumping `enableAutoplay`. The feature is customizable by allowing custom block creators to use their own buttons and text to indicate the autoplay functionality.
