...
const expectDefaultMeta = () => {
	expect(screen.getByText("meta[property='og:site_name']").getAttribute("content")).toEqual(
		websiteName
	);
	expect(screen.getByText("meta[name='twitter:site']").getAttribute("content")).toEqual(
		`@${twitterUsername}`
	);
	expect(screen.getByText("meta[name='twitter:card']").getAttribute("content")).toEqual(
		"summary_large_image"
	);
	expect(screen.getByText("meta[property='fb:admins']").getAttribute("content")).toEqual(
		facebookAdmins
	);
};

const expectDefaultMetaMissing = () => {
	expect(screen.queryByText("meta[name='twitter:site']")).toBeNull();
	expect(screen.getByText("meta[name='twitter:card']").getAttribute("content")).toBe(
		"summary_large_image"
	);
};

const expectImageMetaMissing = () => {
	expect(screen.queryByText("meta[name='twitter:image']")).toBeNull();
	expect(screen.queryByText("meta[name='twitter:image:alt']")).toBeNull();
	expect(screen.queryByText("meta[property='og:image']")).toBeNull();
	expect(screen.queryByText("meta[property='og:image:alt']")).toBeNull();
};
