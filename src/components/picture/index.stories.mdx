...
Image dimensions may be provided as `width` and `height` props. NOTE: The `resizedOptions` will not be used for the height and width; they will be ignored as query parameter for the v2 resizer.
...
** Landscape Picture Source for mobile, portrait Picture Source for desktop **

Multiple `Picture.Source` may be used to explicitly target Art Direction requirements for specific viewport widths.

<Canvas>
	<Story name="Landscape Picture Source for mobile, portrait Picture Source for desktop">
		<Picture>
			<Picture.Source
				src="HY6LDPEW4BBFDLBYD4S3S7LZ3E.jpg"
				width={700}
