...
```jsx
import { Picture, Image } from "@wpmedia/arc-themes-components";

const Feature = () => <Picture>
	<Picture.Source
		src="cat.jpg"
		width={400}
		height={300}
...
A `Picture` with a single `Source` and `Image`.
...
A `Picture` with a multiple `Source` subcomponents to support multiple media queries.
